...
State TrajectorySolver::propagate(const State& initial_state, const Dynamics::DynamicsModel& dynamics,
                                  const TerminationPolicy& termination_policy, double dt, int max_steps,
                                  int& steps_taken, const Integration::Integrator& integrator,
                                  std::function<void(State&, int)> post_step) {
    State current = initial_state;
    steps_taken = 0;
